...
class Reader(BaseReader):
    def read_match(self) -> bMatch:
        ...
        slot_open = self.read_booleans(chio.MULTIPLAYER_MAX_SLOTS)
        slot_used = self.read_booleans(chio.MULTIPLAYER_MAX_SLOTS)
        slot_ready = self.read_booleans(chio.MULTIPLAYER_MAX_SLOTS)

        slot_id = [self.stream.s32() if slot_used[i] else -1 for i in range(chio.MULTIPLAYER_MAX_SLOTS)]
        ...
        slot_status = [
            ...
            for i in range(chio.MULTIPLAYER_MAX_SLOTS)
        ]
        ...
        slots = [
            bSlot(
                slot_id[i],
                slot_status[i],
                [SlotTeam.Neutral for _ in range(chio.MULTIPLAYER_MAX_SLOTS)],
                [Mods.NoMod for _ in range(chio.MULTIPLAYER_MAX_SLOTS)]
            )
            for i in range(chio.MULTIPLAYER_MAX_SLOTS)
        ]
