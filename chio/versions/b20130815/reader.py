...
class Reader(BaseReader):
    ...
    def read_match(self) -> bMatch:
        ...
        slot_status = [SlotStatus(self.stream.u8()) for _ in range(chio.MULTIPLAYER_MAX_SLOTS)]
        slot_team = [SlotTeam(self.stream.u8()) for _ in range(chio.MULTIPLAYER_MAX_SLOTS)]
        slot_id = [
            self.stream.s32()
        ...
        slot_mods = [Mods.NoMod for _ in range(chio.MULTIPLAYER_MAX_SLOTS)]

        try:
            freemod = self.stream.bool()

            if freemod:
                slot_mods = [Mods(self.stream.u32()) for _ in range(chio.MULTIPLAYER_MAX_SLOTS)]
        except OverflowError:
